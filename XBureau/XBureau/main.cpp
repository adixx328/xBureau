...
static XPLMDataRef APstate = NULL;
	...
	char *		outDesc)
{
	...
	strcpy(outName, "XB318AP_and_AT_plugin");
	strcpy(outSig, "318.APAT");
	strcpy(outDesc, "Lubie Placki");

	g_menu_container_idx = XPLMAppendMenuItem(XPLMFindPluginsMenu(), "XMaster 777x", 0, 0);
	g_menu_id = XPLMCreateMenu("AP_and_AT Menu", XPLMFindPluginsMenu(), g_menu_container_idx, menu_handler, NULL);
	XPLMAppendMenuItem(g_menu_id, "Open AP", (void *)"Menu Item 1", 1);
	XPLMAppendMenuItem(g_menu_id, "Toggle FD", (void *)"Menu Item 2", 1);
	XPLMAppendMenuItem(g_menu_id, "Toggle AP", (void *)"Menu Item 3", 1);
	XPLMAppendMenuSeparator(g_menu_id);
	XPLMAppendMenuItem(g_menu_id, "Close AP", (void *)"Menu Item 4", 1);

	//adding datarefs to the program

	APstate = XPLMFindDataRef("sim/cockpit/autopilot/autopilot_mode");
	...
	XPLMSetWindowTitle(g_window, "777x DCDU");
...
void menu_handler(void * in_menu_ref, void * in_item_ref)
{
	...
	{
		if (XPLMGetDatai(APstate) == 0) {
			XPLMSetDatai(APstate, 1);
		}
		else if (XPLMGetDatai(APstate) == 1) {
			XPLMSetDatai(APstate, 0);
		}
	}
...
void	draw(XPLMWindowID in_window_id, void * in_refcon)
{
	...
	// Draw our buttons
	{
		...
		const char * position_btn_text = "TAK";
